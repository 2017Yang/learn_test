...
static inline void __list_splice_add_tail(const struct list_head *list,  
                 struct list_head *prev,  
                 struct list_head *next)
{
    struct list_head *first = list->next;  
...
static inline void list_splice_add_tail(struct list_head *list, struct list_head *head)
{
    if (!list_empty(list)) {
        __list_splice_add_tail(list, head->prev, head); 
    }
}
...
#define list_for_each_entry_safe(pos, n, typeof_type, head, member) \
for (pos = list_entry((head)->next, typeof_type, member), \
	n = list_entry(pos->member.next, typeof_type, member); \
	&pos->member != (head); \
	pos = n, n = list_entry(n->member.next, typeof_type, member))
#endif
struct hlist_head {
